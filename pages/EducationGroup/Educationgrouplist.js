...
const Educationgrouplist = (props) => {
  ...
  const [educationgroupdata, setEducationgroup] = React.useState(null);
...
const loadSurveyData = async () => {
  ...
  let apiResult = await Promise.all([
    supabase.from("education_group").select("id, educationgroup_name"),
    supabase.from("language").select("code, description")
  ]);
  ...
  const getEducationGroupList = async () => {
    const { data, error } = await supabase.from("education_group").select();
    if(error) {
      throw error;
    }
  ...
  return (
    <div className="datatable-crud-demo">
      ...
      <Dialog
        visible={productDialog} style={{ width: '80vw' }} header="Edit Educationgroup" modal className="p-fluid"  onHide={() => setProductDialog(false)}>
          {productDialog ? (
            <Survey 
