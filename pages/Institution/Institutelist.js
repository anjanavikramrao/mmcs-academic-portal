...
const InstituteList = (props) => {
  ...
  const columns = [
    { field: "id", header: "ID" },
    { field: "institution_name", header: "Institution Group Name" },
    { field: "email_id", header: "Email" },
    { field: "date_established", header: "Date Established" },
...
const loadSurveyData = async () => {
  ...
  const getInstituteList = async () => {
    const { data, error } = await supabase.from("campusinstitution").select();
    if(error) {
      throw error;
    }
    setInstitute(data);
  ...
  return (
    <div className="datatable-crud-demo">
      ...
      <Dialog
        visible={productDialog} style={{ width: '80vw' }} header="Edit Institute" modal className="p-fluid"  onHide={() => setProductDialog(false)}>
          {productDialog ? (
            <Survey 
