...
const UniversityHierarchy = (props) => {
  ...
  React.useEffect(() => {
    ...
    const getDetails = async () => {
      let { data, error } = await supabase.rpc(
        "list_of_asscoiation_uninstgroup",
        {
          input_id,
        }
      );
      ...
      places = places.sort((i, j) => i.university_name - j.university_name)
      ...
      places.reduce((acc, place) => {
        ...
        let plc = {id: place.institutiongroup_id, label:place.out_label,expanded:true, data: { name: place.institutiongroup_name }, children: []}
      
        // if there is a parent
        if (place.university_name) {
          // add the current object to the parent
          acc[place.university_name].children.push(plc)
        } else {
          // or add the current object to the root
          result.push(plc)
        }
        // easy acces to this object
        acc[place.institutiongroup_id] = plc
...
export default UniversityHierarchy;
